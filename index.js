...
// ==============================
// CONTROL DE RETROCESO (MÓVIL + DESKTOP) CON MODAL
// ==============================
...
// Estado
window.currentView = "viewFormatos"; 
let allowBackNavigation = true; // Sólo permitido en vista principal

// Detectar si hay datos en inputs
function hayDatosLlenados() {
  const inputs = document.querySelectorAll("input, textarea, select");
  return Array.from(inputs).some(i => i.value && i.value.trim() !== "");
}

// Mostrar modal
function mostrarBloqueoSalida() {
  if (backModal) {
    backModalMsg.textContent = "Termina de llenar el formulario antes de salir.";
...
// Monitorear cambios de vista
document.addEventListener("viewChanged", () => {
  allowBackNavigation = (window.currentView === "viewFormatos");
});

// Interceptar botón atrás (Android + iPhone)
window.addEventListener("popstate", (e) => {
  if (!allowBackNavigation && hayDatosLlenados()) {
    e.preventDefault();
    history.pushState(null, "", location.href); // fuerza quedarse
    mostrarBloqueoSalida();
  }
});
...
inputMarcaReleAutorellenado.addEventListener("input", (e) => {
  ...
  if (diccionarioMarcas[valor]) {
    aplicarDiccionario(valor);
  } else {
    limpiarCamposDiccionario();
  }
});
