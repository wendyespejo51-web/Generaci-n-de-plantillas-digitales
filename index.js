...
function registrarListenersGlobales() {
  ...
  const equipoCA = document.getElementById("equipoCA");
  if (equipoCA) {
    equipoCA.addEventListener("change", () => {
      actualizarCampoCodigo();
      actualizarVisibilidadNodoFinalCA();
    });
  }

  const equipoPS = document.getElementById("equipoPS");
  if (equipoPS) {
    equipoPS.addEventListener("change", actualizarCampoRC);
  }
...
function obtenerFormularioActual(){
  ...
  for(const form in mapaFormularios){

    if(mapaFormularios[form].includes(window.currentView)){
      return form;
    }
...
function limpiarCamposRecomendadosPorVista(vistaId) {
  ...
  const configuracionPorVista = {
    viewParte2: mapaRellenoFormulario1,
    viewCambioAjuste2: mapaRellenoFormulario2
  };
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (datosGuardados && ultimaVista) {
    ...
    const configuracionPorVista = {
      viewParte1: {
        mapa: mapaRellenoFormulario1,
        reglas: reglasCurvaFormulario1
      },
      viewCambioAjuste: {
        mapa: mapaRellenoFormulario2,
        reglas: reglasCurvaFormulario2
      }
    };
...
// =============================================
    ...
    // =============================================
        ...
        if (!document.getElementById("fechaCA").value) {
          inicializarFechaYSemana("fechaCA", "SemanaCA");
        }

        if (!document.getElementById("fechaPS").value) {
...
function registrarListenersFormulario() {
  ...
  const equipo = document.getElementById("equipo");
  if (equipo && !equipo.dataset.listener) {
    equipo.addEventListener("change", actualizarCampoSed);
    equipo.dataset.listener = "true";
  }

  const equipoCA = document.getElementById("equipoCA");
  if (equipoCA && !equipoCA.dataset.listener) {
    equipoCA.addEventListener("change", actualizarCampoCodigo);
    equipoCA.dataset.listener = "true";
  }

  const equipoPS = document.getElementById("equipoPS");
  if (equipoPS && !equipoPS.dataset.listener) {
    equipoPS.addEventListener("change", actualizarCampoRC);
    equipoPS.dataset.listener = "true";
  }
}
...
async function buscarHistorial() {
  ...
  if (!codigo || !tipoFormato) {
    mostrarMensaje("Debe especificar el Nº de activo y seleccionar el tipo de formato", "warning");
    btn.disabled = false;
    return;
  }
...
if (formatoCards && formatoCards.length) {
  formatoCards.forEach(card => {
    ...
    document.getElementById("CodigoUnicoCA").value = "";
    document.getElementById("CodigoUnicoPS").value = "";
...
function restaurarFormulario() {
  ...
  if (window.restauracionEnProceso) return;

  window.restauracionEnProceso = true;

  if (restauracionBloqueada) return;
  ...
  try {
    ...
    data = JSON.parse(
      localStorage.getItem('formulario_draft_' + formActual)
    );

  } catch(e) {

    console.warn("Error leyendo draft");
  ...
  Object.entries(data).forEach(([key, value]) => {
    ...
    elementos.forEach(el => {
      ...
      // 🔥 CASO ESPECIAL: SELECT QUE PUEDE CAMBIAR A INPUT
      if (el.tagName === "SELECT") {

      const existeEnOpciones = Array.from(el.options)
        .some(opt => opt.value === value);

      if (!existeEnOpciones && value) {

        const wrapper = el.closest("[id^='wrapper']");
        const btnVolver = wrapper ? wrapper.querySelector("button") : null;
...
// =======================================
// 2️⃣ MAPEO FORMULARIO 2 (NUEVO)
// =======================================
const mapaRellenoFormulario2 = {
  "51P": {
...
// Funcion para mensajes tipo toast
function mostrarMensaje(mensaje, tipo = "success") {
  const toast = document.createElement("div");
  toast.className = `toast ${tipo}`;
  toast.textContent = mensaje;
...
async function enviarFotosRegistro() {
  ...
  for (const id of camposFotos){
    ...
    const codigoLimpio = metadata.CodigoUnico.replace(/\D/g,"");
    const celdaLimpia = metadata.celda === "-" ? "" : "Celda" + metadata.celda;
...
document.getElementById("enviarBtn").addEventListener("click", (e) => {
  ...
  enviarFormulario(
    ...
    [
      ...
      "modelorele","serierele","seriereleSAP","CapacidadR","CapacidadT","VnRele","AlimentacionRele","AnioFabricacionRele","kardexequipo","marcaequipo","modeloequipo","Tiporele","serieequipo",
...
if (btnEnviarParte3) {
  btnEnviarParte3.addEventListener("click", (e) => {
    ...
    enviarFormulario(
      ...
      "Nombre1","CodigoLDS1","Nombre2","CodigoLDS2","Correo1","Correo2","fotoPlacaRele","fotoPlacaEquipoMT","fotoPuertaCelda", "fotoCeldaInterior","fotoGabinete"
      ],
      'https://default1c0051dd45964b1a9849d060735057.69.environment.api.powerplatform.com:443/powerautomate/automations/direct/workflows/e30b9abb12584fbaa6939129547bb248/triggers/manual/paths/invoke?api-version=1&sp=%2Ftriggers%2Fmanual%2Frun&sv=1.0&sig=2ONLDOmbTWMdxkm4zDsj2Sd0P7zcPx1078HJmoEHMlo',   
      camposChecklistPreguntas, // <--- Pasamos el array aquí
      true
    );
  });
}
...
document.getElementById("enviarBtn3").addEventListener("click", (e) => {
    ...
    enviarFormulario(
        e.target,
        [
          "CodigoUnicoPS", "TensionRC", "AlimentadorRC", "RC", "equipoPS","CircuitoPS", "PDS", "fechaPS", "SemanaPS", "horaPS", "ProtocoloRC", "Nombre1PS", "Correo1PS", "Nombre2PS", "Correo2PS", "MarcaRC", "CapacidadRC", "CapRupturaRC", "MExtinsionRC", "NSerieRC", "NSerieRCSAP","TNominalRC", "FuenteCargaRC", "SecuenciaFasesRC","AnoRC", "Kardex1RC", "Kardex2RC", "MarcaPS", "ModeloReleRC", "NSerieReleRC", "NSerieReleRCSAP",
