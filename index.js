...
if (nextButton) {
    nextButton.addEventListener("click", () => {
        ...
        const vistaActiva = document.querySelector(".view.active-view");
        ...
        if (selectedPdf === "Protocolodepruebas.pdf") {
            ...
            if (
              ...
              // 🔹 Cambiar label
              labelSed.textContent = "Codigo de equipo MT";

              // 🔥 LIMPIAR sed para evitar prefijos con valores antiguos
              if (inputsed) {
                inputsed.value = "";
                inputsed.type = "number";     // 👈 forzar numérico
              }

              // 🔹 DESACTIVAR autocomplete REAL
              ...
              if (inputsed) {
                inputsed.value = "";
                inputsed.type = "text";   // 👈 vuelve a texto
              }

              // 🔹 Mostrar lista
              suggestionsSed.style.display = "";

              // 🔹 Reactivar autocomplete
        ...
        // =============================================
          ...
          function actualizarCampoCodigo() {
            ...
            if (esEquipoMT) {
              // 🔹 Cambiar label
              labelCodigo.textContent = "Codigo de equipo MT";

              // 🔥 LIMPIAR codigo para evitar prefijos con valores antiguos
              if (inputcodigo) {
                inputcodigo.value = "";
                inputcodigo.type = "number";     // 👈 forzar numérico
              }
              ...
              if (inputcodigo) {
                inputcodigo.value = "";
                inputcodigo.type = "text";   // 👈 vuelve a texto
              }
...
function actualizarConcatenacion() {
  const sed = sedinput ? sedinput.value.trim() : "";
  const equipo = document.getElementById("equipo")?.value || "";

  let nodofinal = "";

  // 👇 Solo toma nodofinal si existe Y está visible
  if (
    NodoFinalInput &&
    NodoFinalInput.value &&
    NodoFinalInput.offsetParent !== null
  ) {
    nodofinal = NodoFinalInput.value.trim();
  }

  if (!enlaceInput) return;

  // 🔹 Si no hay SED, limpiar
  if (!sed) {
    enlaceInput.value = "";
    return;
  }

  // 🔥 CASOS SIN NODO FINAL → prefijos
  if (!nodofinal) {
    if (equipo === "Recloser") {
    ...
    if (equipo === "Seccionador de potencia con telemando") {
      enlaceInput.value = "SP" + sed;
      return;
    }
  }

  // 🔹 CASO NORMAL (SED + NodoFinal)
...
function actualizarConcatenacionCambioAjuste() {
  ...
  const equipoCA = document.getElementById("equipoCA")?.value || "";
  ...
  if (
    ...
    codigofinal = codigofinalInput.value.trim();
  }
  ...
  // 🔥 CASOS SIN NODO FINAL → prefijo
  if (!codigofinal) {
    if (equipoCA === "Recloser") {
      valorFinal = "RC" + codigo;
    } 
    else if (equipoCA === "Seccionador de potencia con telemando") {
  ...
  // 🔹 CircuitoCA
  if (circuitoInput) {
    circuitoInput.value = valorFinal;
  }

  // 🔹 enlaceCA
  if (enlaceCAInput) {
    enlaceCAInput.value = valorFinal;
  }
}
...
async function obtenerDatosDesdeFlujo(payload) {
  const urlFlujo = "https://default1c0051dd45964b1a9849d060735057.69.environment.api.powerplatform.com:443/powerautomate/automations/direct/workflows/2814051425604f6290304a11b63686cf/triggers/manual/paths/invoke?api-version=1&sp=%2Ftriggers%2Fmanual%2Frun&sv=1.0&sig=7lR5vpJvXZsqk05SXTMvHYuxoBTkMbhjnII6FNdXEGU"; // tu URL real
...
// Desactivar resaltado del combobox ANSI
//function desactivarResaltadoANSI() {
 // if (!opcionesCombobox) return;
...
document.querySelectorAll('.btn-next').forEach(btn => {
  ...
  btn.addEventListener('click', async e => {   // 🔥 async AQUÍ
    ...
    try {
      ...
      setLoaderProgress(30); // request enviado
...
function limpiarFormulario() {
  ...
  vistas.forEach(vista => {
    ...
    if (vista.id.startsWith("viewParte")) {
      ...
      camposExcluidos = ["CodigoUnicoPS", "fechaPS", "SemanaPS", "horaPS"];
    }
